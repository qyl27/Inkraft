package cx.rain.mc.inkraft.mod.data;
...
public class InkReloadListener implements PreparableReloadListener {
    ...
    public static final ResourceLocation INKRAFT_STORY = new ResourceLocation(InkraftMod.ID, STORY_PATH);
    ...
    private void apply(Map<ResourceLocation, String> stories, ProfilerFiller reloadProfiler) {
        ...
        InkraftMod.getInstance().getStoriesManager().clearStories();

        for (var story : stories.entrySet()) {
            InkraftMod.getInstance().getStoriesManager().addStory(story.getKey(), story.getValue());
        }
