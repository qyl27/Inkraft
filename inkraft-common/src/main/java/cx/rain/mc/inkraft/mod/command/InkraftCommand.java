package cx.rain.mc.inkraft.mod.command;
...
public class InkraftCommand {
    ...
    private static int onVersion(final CommandContext<CommandSourceStack> context) {
        context.getSource().sendSuccess(() ->
                Component.literal("Inkraft ver: " + InkraftMod.VERSION).withStyle(ChatFormatting.AQUA), true);
        return 1;
    }
    ...
    private static int onClearState(CommandContext<CommandSourceStack> context) {
        ...
        InkraftMod.getInstance().getStoriesManager().refreshStory(player);
    ...
    private static void startStory(ServerPlayer player, ResourceLocation path, IStoryHolder stateHolder, boolean isDebug) {
        var storiesManager = InkraftMod.getInstance().getStoriesManager();
    ...
    private static CompletableFuture<Suggestions> suggestStart(final CommandContext<CommandSourceStack> context,
        ...
        for (var story : InkraftMod.getInstance().getStoriesManager().getLoadedStories()) {
